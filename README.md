...
- авторизация в web portal через Telegram-бота и Telegram Mini App
- синхронизация сайта и бота через одну БД и один `telegram_id`
- trial на 2 дня
- тариф `Start` за `30 ₽ / 7 дней` или `69 ₽ / 30 дней`
- тариф `Pro` за `199 ₽ / 30 дней`
- тариф `Pro` за `65 ₽ / 7 дней`
- списание полной стоимости при активации и каждом продлении периода
...
- для тарифа `Start`: один автоматически назначенный сервер + доступ к `Белым спискам`
- в тарифы включён объём трафика `Белых списков`, после него применяется цена `2 ₽ / ГБ`
- доступны отдельные пакеты дополнительного трафика `Белых списков`
- для тарифа `Pro`: доступ ко всем активным серверам
...
- тариф `Start` получает один назначенный сервер своего типа
- whitelist-доступ выдаётся отдельно
- `Pro` получает все активные серверы
