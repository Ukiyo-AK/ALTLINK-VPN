"""remove Start whitelist debt wording from plan descriptions
        ...
        sa.text(
            ...
            SET description = CASE CAST(code AS TEXT)
                WHEN 'single_10gbit' THEN
                    'Один автоматически назначенный 10 Гбит сервер. Серверы типа «Белые списки» доступны отдельно и тарифицируются по 2 ₽ за ГБ. Лимит — 2 устройства.'
                WHEN 'single_10gbit_weekly' THEN
                    'Тот же доступ к 10 Гбит серверу, но с еженедельным списанием. Серверы типа «Белые списки» доступны отдельно и тарифицируются по 2 ₽ за ГБ. В пересчёте на месяц стоит на 30% дороже. Лимит — 2 устройства.'
                ELSE description
            END
