...
Замените все значения `change-me*` в секретах на независимые случайные строки. Известный
`SESSION_SECRET_KEY` позволяет подделывать веб-сессии. Административный API не принимает
пустой ключ или значение `change-me-admin-api-key` и возвращает 503 до настройки ключа.
Для HTTPS укажите `BACKEND_PUBLIC_URL=https://...`: cookie сессии будет `Secure` и `HttpOnly`.

Если реквизиты YooKassa не заполнены, доступно только ручное подтверждение оплаты:
автоматического зачисления тестовых денег больше нет. `PAYMENT_PROVIDER=stub` предназначен
только для разработки; при `ENVIRONMENT=production` он также заменяется ручной оплатой.
...
В интерфейсе сохраняются два пользовательских адреса: исходная ссылка Remnawave
для ручного импорта и страница быстрого подключения
`BACKEND_PUBLIC_URL/connect/<short_uuid>` с кнопками Happ и INCY. Страница получает
конфигурацию через техническое зеркало `BACKEND_PUBLIC_URL/sub/<short_uuid>`.
Не указывайте `BACKEND_PUBLIC_URL` в `REMNAWAVE_SUBSCRIPTION_BASE_URL`, иначе
зеркало будет ссылаться само на себя.
Если указан только домен панели, зеркало использует путь `/api/sub/<short_uuid>`.
Если публичная подписка доступна по другому префиксу, задайте его целиком,
например `REMNAWAVE_SUBSCRIPTION_BASE_URL=https://sub.example.com/s`.
Зеркало не хранит конфиги: при обновлении в клиенте оно обращается к Remnawave.
Перевыпуск ссылки меняет токен обоих адресов, старые адреса перестают работать.
...
## 17. Автопродление: миграция 20260914_0019

Пользовательские переключатели и статус автопродления скрыты. Админские данные остаются доступны.
Миграция включает `auto_renew` у существующих платных подписок, включая исторические записи,
но не меняет их статус, срок, баланс или доступ. Тестовые подписки не затрагиваются.
Поэтому завершённые подписки не активируются повторно, а действующие продлеваются обычным
биллингом в срок при достаточном балансе. Просроченная активная подписка может продлиться
при следующем запуске биллинга. Новые платные подписки уже создаются с `auto_renew=True`.

Перед обновлением сделайте резервную копию БД. Остановите обработчики на время миграции,
чтобы старый scheduler не отправил устаревшее уведомление и старый бот не изменил настройку:

```bash
docker compose build
docker compose stop scheduler client-bot admin-bot backend
docker compose run --rm --no-deps backend python -m alembic upgrade head
docker compose up -d
```

После успешной миграции можно проверить `docker compose exec backend python -m alembic current`:
ожидается `20260914_0019 (head)`. Записи уже отправленных сообщений сохраняются;
устаревшие ожидающие напоминания об отключённом продлении помечаются как отменённые
через статус `failed` с причиной `Superseded by renewal policy migration 20260914_0019`.
Откат миграции не отключает продление всем пользователям: исходные индивидуальные
настройки можно восстановить только из резервной копии.
